...
<head>
  <meta charset="UTF-8">
  <title>Create WooCommerce Order</title>
...
<div class="container mx-auto p-6">
  ...
  <form action="{{ route('order') }}" method="post" class="bg-white p-6 rounded-lg shadow-md">
    @csrf
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <input type="text" name="address_1" required class="w-full border border-gray-300 rounded-lg p-2">
      </div>
      <div>
      ...
      <div>
        ...
        <input type="text" name="state" value="notset" class="w-full border border-gray-300 rounded-lg p-2">
      </div>
      <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <select name="payment_method" id="payment_method" required onchange="updatePaymentMethodTitle()" class="w-full border border-gray-300 rounded-lg p-2">
      ...
      <div>
        ...
        <select name="payment_method_title" id="payment_method_title" required class="w-full border border-gray-300 rounded-lg p-2">
    ...
    <button type="button" onclick="openModal()" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600">Search & Add Product</button>
    ...
    <button type="submit" class="mt-6 bg-green-500 text-white px-6 py-2 rounded-lg hover:bg-green-600">Create Order</button>
  </form>
</div>
...
<div id="searchProductModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
  <div class="bg-white w-1/2 p-6 rounded-lg shadow-lg">
    ...
    <form id="searchProductForm" onsubmit="return false;">
      ...
      <input type="text" id="productName" name="product_name" required oninput="debouncedSearch()" class="w-full border border-gray-300 rounded-lg p-2 mt-2 mb-4">
      <button type="button" onclick="closeModal()" class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600">Close</button>
    </form>
    ...
    fetch(`/products/${encodeURIComponent(productName)}`)
      ...
      .then(data => {
        ...
        if (!data || data.length === 0 || data.message === 'No products found') {
          ...
          let resultHTML = `<p>Results for "<strong>${productName}</strong>":</p><ul style="list-style:none; padding:0;">`;

          data.forEach(product => {
            const imageUrl = product.images.length ? product.images[0].src : 'https://via.placeholder.com/100?text=No+Image';
      ...
      .catch(error => {
        ...
        document.getElementById('searchResults').innerHTML = '<p style="color:red;">Error fetching products. Please try again later.</p>';
      });
  }
